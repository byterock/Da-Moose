...
sub create {
   ...
   $self->_operation(DBIx::DA::Constants::DA::CREATE);
   return $self->_execute($dbh,$container);
}
sub retrieve {
   my $self=shift;
   my ($dbh,$container,$opt) = @_;
   $self->_operation(DBIx::DA::Constants::DA::RETRIEVE);
...
sub update {
   ...
   die "error: ".ref($self).", Attempt to do an update operation when can_modify is false!"
      unless ($self->can_modify());
   $self->_operation(DBIx::DA::Constants::DA::UPDATE);
   return $self->_execute($dbh,$container);
}
sub delete {
   my $self=shift;
   ...
   die "error: ".ref($self).", Attempt to do a delete operation when can_modify is false!"
      unless ($self->can_modify());
   $self->_operation(DBIx::DA::Constants::DA::DELETE);
   return $self->_execute($dbh,$container);
}
